...
const authStationUserController = {
  async login(req, res) {
    await StationUser.findOne({ name: req.body.name })
      .then(response => {
        ...
        bcrypt.compare(
          ...
          async (err, result) => {
            if (err) {
              ...
              await StationUser.findByIdAndUpdate(response.id, {
                $set: {
...
module.exports = authStationUserController;
